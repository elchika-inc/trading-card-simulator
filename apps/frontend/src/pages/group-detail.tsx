import type { PackGroupWithPacks } from "@repo/types";
import { PackVisual } from "@repo/ui/pack-visual";
import { ChevronLeft, ChevronRight, Coins, FolderOpen, Loader2 } from "lucide-react";
import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import { CoinDisplay } from "@/components/app/pack-select/coin-display";
import { PageLayout } from "@/components/app/page-layout";
import { useUser } from "@/contexts/user-context";
import { getPackGroup } from "@/lib/api-client";

/**
 * グループ詳細ページ（グループ内パック一覧）
 * /groups/:groupId
 */
export function GroupDetail() {
  ...
  const { coins } = useUser();
  const [group, setGroup] = useState<PackGroupWithPacks | null>(null);
  ...
  useEffect(() => {
    ...
    getPackGroup(groupId).then((data) => {
      setGroup(data);
      setLoading(false);
      // 初期選択を中央に
      if (data && data.packs.length > 0) {
        setActiveIndex(Math.floor(data.packs.length / 2));
      }
    });
  }, [groupId]);
    ...
    return (
      <PageLayout>
        <div className="min-h-screen flex flex-col items-center justify-center gap-4">
          <p className="text-zinc-400">グループが見つかりません</p>
          <button
  ...
  if (packs.length === 0) {
    return (
      <PageLayout>
        <div className="min-h-screen flex flex-col items-center justify-center gap-4">
          <p className="text-zinc-400">このグループにパックがありません</p>
          <button
            onClick={() => navigate("/groups")}
            className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors"
            type="button"
    ...
    <PageLayout>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center select-none relative">
        ...
        <div className="absolute top-0 left-0 right-0 p-6 flex justify-start items-center z-20">
          ...
          >
            <FolderOpen size={20} />
            <span className="text-sm font-bold">シリーズ選択に戻る</span>
          </button>
        </div>
        ...
        >
          <div
            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r ${group.colorFrom} ${group.colorTo} text-white text-xs font-medium border border-white/20`}
          >
