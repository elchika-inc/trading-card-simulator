...
import { Sparkles, Hand, ChevronsRight } from 'lucide-react';
import { CardView } from './CardView';
import { INSPECT_CARD_SIZE } from '../utils/constants';

/**
 * 1枚ずつ確認モードコンポーネント
 */
export const PackInspecting = ({
  gameState,
  cards,
  currentCardIndex,
  isCardFlipped,
  ...
  return (
    <div className="relative cursor-pointer perspective-[1200px]" onClick={onCardClick}>
      {/* 1枚確認時は大きなサイズを使用 */}
      <div className={`relative ${INSPECT_CARD_SIZE}`}>
        <div
          ...
          `}
          style={{
            transition: 'transform 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275), opacity 0.3s, scale 0.3s'
          }}
        >
          ...
          <div
            className="absolute inset-0 backface-hidden rotate-y-180 rounded-xl overflow-hidden shadow-2xl border-[4px] border-slate-700 bg-slate-800"
          >
            {backImage ? (
              <img
                src={backImage}
