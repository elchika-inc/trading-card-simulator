...
import { FALLBACK_GRADIENT } from '../utils/constants';

/**
 * 未開封パック表示コンポーネント
 */
export const PackIdle = ({
  packRef,
  gameState,
  tearProgress,
  packImage,
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          clipPath: 'polygon(0% 0%, 0% 70px, 10% 60px, 20% 70px, 30% 60px, 40% 70px, 50% 60px, 60% 70px, 70% 60px, 80% 70px, 90% 60px, 100% 70px, 100% 100%, 0% 100%)'
        }}
      >
        <div className="w-full h-full bg-slate-800 relative">
          ...
          <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/10 to-white/30 pointer-events-none mix-blend-overlay"></div>
          <div className="absolute inset-0 bg-gradient-to-b from-black/0 via-black/0 to-black/60 pointer-events-none"></div>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          `,
          opacity: 1 - Math.pow(tearProgress, 3),
      ...
      >
        <div className="w-full h-[440px] relative -top-[0px]">
          ...
          />
          <div className="absolute inset-0 -z-10" style={{ background: FALLBACK_GRADIENT }} />
          <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/20 to-white/40 pointer-events-none mix-blend-overlay"></div>
        </div>
        ...
        {gameState === 'idle' && tearProgress < 0.1 && (
          <div className="absolute bottom-4 left-0 right-0 h-[2px] border-b-2 border-dashed border-white/60 opacity-80 z-50 pointer-events-none">
            <div className="absolute inset-0 h-full w-[30%] bg-gradient-to-r from-transparent via-white to-transparent animate-shimmer-fast"></div>
          </div>
        )}
      </div>
      ...
      {gameState === 'opening' && (
        <div className="absolute top-0 left-0 right-0 h-[2px] z-40">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[500px] bg-white blur-[40px] animate-flash"></div>
        </div>
      )}
    </div>
  );
