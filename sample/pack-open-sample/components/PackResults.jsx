 ...
 * 結果一覧モードコンポーネント
 */
export const PackResults = ({ gameState, cards, onReset }) => {
  ...
  return (
    <div className="absolute inset-0 z-[100] animate-fade-in pointer-events-auto flex items-center justify-center">
      <div className="w-full h-full flex flex-col items-center justify-center p-4">
        ...
        <div className="flex flex-col items-center justify-center gap-4 w-full">
          ...
          <div className="flex flex-wrap justify-center items-end gap-2 sm:gap-6">
            {cards.slice(0, 3).map((card, index) => (
              ...
              >
                <CardView card={card} borderWidth="border-[2px] md:border-[3px]" />
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 flex flex-col items-center gap-4 animate-fade-in" style={{ animationDelay: '800ms', animationFillMode: 'both' }}>
          <h2 className="text-white text-xl md:text-2xl font-bold tracking-widest drop-shadow-lg">NEW CARDS!</h2>
          <button
            onClick={onReset}
