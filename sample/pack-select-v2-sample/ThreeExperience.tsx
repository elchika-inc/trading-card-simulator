...
import { SoundEffects, type CardAsset, type Assets } from './constants';
...
export const ThreeExperience: React.FC<ThreeExperienceProps> = ({
  cards,
  assets,
  onComplete
}) => {
  const mountRef = React.useRef<HTMLDivElement>(null);
  const [statusText, setStatusText] = React.useState("スワイプで選んでタップ");
  const [cursorClass, setCursorClass] = React.useState("cursor-grab");

  // Refs
  const stateRef = React.useRef('SELECTION'); // SELECTION, SELECT_ANIM, PACK_IDLE, ...
  const isInteractableRef = React.useRef(true);
  const startTimeRef = React.useRef(0);

  // Selection specific refs
  const carouselGroupRef = React.useRef<THREE.Group | null>(null);
  const packMeshesRef = React.useRef<THREE.Group[]>([]);
  const carouselRotationRef = React.useRef(0); // Current rotation Y
  const targetCarouselRotationRef = React.useRef(0);

  // Opening/Card specific refs
  const mainPackGroupRef = React.useRef<THREE.Group | null>(null);
  const startRotationRef = React.useRef({ x: 0, y: 0 });
  const packRotationRef = React.useRef({ x: 0, y: 0 }); // Idle tilt
  ...
  React.useEffect(() => {
    ...
    const packBackMat = new THREE.MeshBasicMaterial({ map: dummyBackTex, side: THREE.FrontSide });
    ...
    const packInsideMat = new THREE.MeshBasicMaterial({ color: 0x1a1a1a, side: THREE.FrontSide });
    ...
    // Bottom
    const botGeo = createPillowGeometry(packW, botH, packDepth, { openTop: true });
    const botMat = new THREE.MeshBasicMaterial({ map: botTex, side: THREE.FrontSide });
    ...
    // Bottom Inside
    const botInsideMat = new THREE.MeshBasicMaterial({ color: 0x1a1a1a, side: THREE.FrontSide });
    ...
    // Top
    const topGeo = createPillowGeometry(packW, topH, packDepth, { openBottom: true });
    ...
    // Top Inside
    const topInsideMat = new THREE.MeshBasicMaterial({ color: 0x1a1a1a, side: THREE.FrontSide });
    ...
    cards.forEach((data, i) => {
      ...
      mesh.userData = { index: i, isRare: data.isRare, state: 'deck', originalZ: -i * 0.02 };
      cardsGroup.add(mesh);
      meshes.push(mesh);
    });
    ...
    const handleInteractionMove = (clientX: number, clientY: number) => {
      ...
      mouseRef.current = { x: (clientX / w) * 2 - 1, y: -(clientY / h) * 2 + 1 };
    ...
    const handleTapAction = () => {
      if (stateRef.current === 'SELECTION') {
        ...
        let index = Math.round(-carouselGroupRef.current!.rotation.y / seg) % packCount;
        ...
        startRotationRef.current = { x: mainPackGroup.rotation.x, y: mainPackGroup.rotation.y };
        ...
        if (card.userData.state === 'deck') {
          ...
          if (currentCardIndexRef.current >= meshes.length) setTimeout(onComplete, 800);
        }
      }
    };
    ...
    const animate = () => {
      ...
      // 0. Selection Phase
      if (stateRef.current === 'SELECTION') {
        carouselGroup.rotation.y += (targetCarouselRotationRef.current - carouselGroup.rotation.y) * 0.1;
      ...
      if (stateRef.current === 'SELECT_ANIM') {
        ...
        carouselGroup.rotation.y += (targetCarouselRotationRef.current - carouselGroup.rotation.y) * 0.2;
  ...
  return (
    ...
    >
      <div className="absolute bottom-10 left-0 w-full text-center pointer-events-none select-none">
        <p className={`text-gray-300 text-lg font-bold drop-shadow-lg transition-opacity duration-500 ${statusText ? 'opacity-100' : 'opacity-0'}`}>
          {statusText}
        </p>
      </div>
    </div>
