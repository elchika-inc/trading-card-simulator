...
import { Sparkles, Layers, Trophy, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
import { featuredCards } from './data/featured-cards';
import { cssStyles } from './styles/animations';
        ...
        <div className="flex flex-col items-center perspective-1000 w-full max-w-[400px] lg:max-w-none">
          ...
          <div className="relative w-full flex flex-col items-center">
            ...
            <div className="flex items-center justify-center gap-4 sm:gap-8 w-full z-10">
              ...
              {/* カード配置コンテナ */}
              {/* 修正: 親コンテナで浮遊アニメーションを行い、子の配置に影響を与えないようにする */}
              <div className="relative w-[300px] h-[420px]">
                ...
                <div className="w-full h-full animate-float">
                  ...
                  {featuredCards.map((card, idx) => {
                    ...
                    const prevIndex = (currentIndex - direction + featuredCards.length) % featuredCards.length;
                    ...
                    return (
                      ...
                      >
                        {/* 遷移中は isHoverEnabled=false を渡して3D傾きを無効化 */}
                        <GameCard data={card} isHoverEnabled={!transitioning} />
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="hidden lg:block mt-8 text-center max-w-sm fade-in-up">
            ...
            <h2 className="text-3xl font-bold text-white mb-2 tracking-tight">新パック「虚空の覇者」登場</h2>
          ...
          <div className="mt-4 p-4 rounded-xl bg-black/40 border border-white/5 backdrop-blur-sm">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs group cursor-pointer">
                <span className="text-gray-400 group-hover:text-white transition-colors">1. 世界大会予選開始！</span>
                <ArrowRight className="w-3 h-3 text-gray-600 group-hover:text-purple-400 transition-colors" />
              </div>
              <div className="flex items-center justify-between text-xs group cursor-pointer">
                <span className="text-gray-400 group-hover:text-white transition-colors">2. メンテナンスのお知らせ</span>
                <ArrowRight className="w-3 h-3 text-gray-600 group-hover:text-purple-400 transition-colors" />
              </div>
            </div>
          </div>
