...
import { getHoloStyle } from '../styles/holo-styles';
import { getTextStyle } from '../styles/text-styles';
 ...
 */
export const HoloCard = ({ card, onClick, className }) => {
  ...
  // 座標計算ロジック（マウス/タッチ共通）
  const updateCardState = useCallback((clientX, clientY) => {
  ...
  // イベントハンドラ
  const handleMouseMove = (e) => updateCardState(e.clientX, e.clientY);
  const handleTouchMove = (e) => updateCardState(e.touches[0].clientX, e.touches[0].clientY);
  ...
  return (
    <div
      className={`relative group perspective-1000 touch-none ${className}`} // classNameを適用
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 z-10 flex flex-col pointer-events-none">
          ...
          {/* 枠線 */}
          <div className={`absolute inset-0 border-[6px] rounded-xl border-opacity-60 z-20 pointer-events-none transition-colors duration-300 ${state.isHovering ? 'border-gray-500' : 'border-gray-800'}`} />
          ...
          <div className="flex-1 bg-gray-900 p-4 text-white relative overflow-hidden">
             <div className="relative z-10 flex flex-col h-full">
               <div className="flex justify-between items-start mb-2">
                 <div>
                   <h2 className="text-lg font-bold tracking-wide font-sans mb-0.5" style={textStyle}>
                     {card.name}
                   </h2>
                   <p className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{card.type}</p>
                 </div>
               ...
               <p className="text-xs text-gray-400 leading-relaxed opacity-90">
                 {card.description}
               </p>
             </div>
          </div>
        ...
        <div
          ...
          style={{ boxShadow: `inset 0 0 15px rgba(255,255,255, ${state.isHovering ? 0.15 : 0})` }}
        />
      </div>
    </div>
  );
